...
1. 什么是函数式编程范式？
   函数式编程是一种编程范式，它将计算视为数学函数的求值过程，强调将计算过程分解为一系列纯函数的组合。在函数式编程中，函数被视为一等公民，可以作为参数传递给其他函数，也可以作为返回值返回。
2. 什么是lambda函数？请举例说明。
   Lambda函数是一种匿名函数，它在定义时不需要指定函数名，可以直接在需要使用的地方定义并调用。它是函数式编程中常用的一种技术，特别适用于需要定义简单、易读且只使用一次的函数。
3. 什么是高阶函数？常用的高阶函数有哪些？这些高阶函数如何工作？使用简单的代码示例说明。
   高阶函数是指可以接受一个或多个函数作为参数，并且/或者返回一个函数作为结果的函数。
